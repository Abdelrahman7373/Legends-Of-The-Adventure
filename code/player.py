...
class Player(Entity):
	def __init__(self, pos, groups, obstacle_sprites, create_attack, destroy_attack, create_magic):
		...
		self.image = pygame.image.load('../graphics/player_rock/player.png').convert_alpha()
		...
		self.switch_duration_coldown = 200
		#magic
		...
		self.max_stats = {'health': 1000, 'energy': 1000, 'attack': 100, 'magic': 100, 'speed': 15}
	...
	def cooldowns(self):
		...
		if not self.switch_weapon:
			if current_time - self.weapon_switch_time >= self.switch_duration_coldown:
				self.switch_weapon = True

		if not self.switch_magic:
			if current_time - self.magic_switch_time >= self.switch_duration_coldown:
				self.switch_magic = True
